...
Armed with functional generalisations of the thermodynamic potentials we can describe the inhomogeneous liquid; in the next section we will introduce the main principle underlying DFT.

\subsection{Variational principle}

In the previous section we defined the thermodynamic potentials for an inhomogeneous system in equilibrium with density profile $\rho(\vec{r}) = \rho^{(1)}(\vec{r})$.
...
\marginfootnote{A full account of the classical arguments can be found in Refs.\ \cite{MerminPR1965,EvansAP1979}, and a more compact argument has recently been formulated in Ref.\ \cite{DwandaruPRE2011}.}:
\begin{enumerate}
  \item It is bounded from below by the grand potential, i.e.\
    \begin{equation*}
      \Omega[\rho(\vec{r})] \ge \Omega[\rho^{(1)}(\vec{r})].
    \end{equation*}
...
\begin{equation}\label{eq:dft-equilibrium}
  ...
  0.
\end{equation}
This variational principle provides a route to numerical applications of DFT: the functional form of the grand potential can be minimised to obtain the equilibrium density profile and its corresponding free energy.
Importantly, this procedure can be applied for arbitrary external potentials which could represent e.g.\ a test-particle inserted into the liquid.
...
Our focus will be on correlation within the equilibrium liquid, so in the next section we will directly connect the thermodynamic functionals to the correlations functions.
...
\end{equation}
\todo{Rework this into the section.}
...
It is straightforward to generalise the virial series to inhomogeneous systems, by bringing the density terms inside the cluster integrals and replacing them with their local equivalent i.e.\ $\rho^n \to \rho(\vec{r}_1) \cdots \rho(\vec{r}_n)$ \cite{StillingerJCP1962,RowlinsonPRSA1985}.
