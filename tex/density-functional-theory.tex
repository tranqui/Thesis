...
We will work primarily from the classic texts Refs.\ \cite{EvansAP1979,Evans1989,EvansFoIF1992}, although we found Refs.\ \cite{Ashcroft1996,Hansen2013} helpful supplementary texts.

Our stated goal in this section is to provide more exposition on the various correlation functions important for liquid state physics, as well as the complete historical context of the \emph{morphometric approach} which will be the focus of chapters \ref{chapter:morphometric-framework}, \ref{chapter:morphometric-applications} and \ref{chapter:resummation}.
For the latter goal we will introduce fundamental measure theory in section \ref{sec:fmt} which is its historical antecedent.
The morphometric approach will be properly described in the results chapters, this is merely to provide additional context.

\todo{Edit this section}

\subsection{Inhomogeneous generalisations of the thermodynamic potentials}

Here we introduce the inhomogeneous generalisations of the thermodynamic potentials introduced in section \ref{sec:stat-mech}.
The generalisations will suggest a route to an inhomogeneous theory of fluids.
\todo{Review/edit this opener.}
...
With functional generalisations we can now...
\todo{Write closer}
...
\begin{enumerate}
  ...
  \item with equality only in the case of the equilibrium density profile, i.e.\
    ...
    \end{equation}
\end{enumerate}
\todo{Mermin-Evans argument, or simplified argument (Schmidt).}
...
Henceforth we will only deduce
...
\end{equation}
\todo{Move this until later}

\todo{Add closing paragraph}

\subsection{Thermodynamic potentials as generating functionals}

We now connect the formalism of DFT with the various correlation functions in the liquid.
...
\marginfootnote{Conventionally this class of theories are presented in their own right rather than as a special case of density functional theory; we opted for this more modern presentation to be marginally more economical with chapter length.}.
...
Correspondence with the virial series comes by we note that
\begin{equation*}
  \beta \mathcal{F}^\mathrm{ex} = \int_V \beta f^\mathrm{ex} \, d\vec{r}
\end{equation*}
To introduce the topic we will first consider the free energy density of the homogeneous system, and then generalise to the inhomogeneous one.
From the virial series for an $m$-component mixture \eqref{eq:virial-coefficients-mixtures}, the dilute limit free energy density in the low density limit becomes
\begin{equation*}
  \beta f^\mathrm{ex}
  =
  - \frac{\rho^2}{2}
...
It is straightforward to generalise the virial series to inhomogeneous systems, by bringing the density terms inside the cluster integrals with their local equivalent i.e.\ $\rho^n \to \rho(\vec{r}_1) \cdots \rho(\vec{r}_n)$ \cite{StillingerJCP1962}.
\todo{There should be another reference for the virial series}
...
\begin{equation*}
  ...
  + \mathcal{O}(\rho^2).
\end{equation*}
Fortuitously, all of the molarity information has been absorbed into the definition of the weighted densities, so the final form contains no explicit mixture dependence.
...
\end{equation}
where their values in the bulk, the so-called the \emph{scaled particle variables}, can be obtained from \eqref{eq:fmt-n-derivatives} as
\begin{subequations}\label{eq:spt-variables}
  ...
  \end{align}
\end{subequations}
\todo{Connect to truncatable free energies.}
...
The liquid is metastable above the melting transition, so high density simulations require the introduction of polydispersity to prevent crystallisation; to determine its accuracy the CS equation has to be extended to mixtures.
...
\begin{SCfigure}
  \includegraphics[width=\linewidth,outer]{swap-cs-santos}
...
\todo{Generalisations to non-spherical systems.}
