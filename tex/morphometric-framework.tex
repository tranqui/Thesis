...
\begin{tcolorbox}[title=Virial expansion of the pressure]
The virial series for the equation of state is%
\marginfootnote{The left-hand side of \eqref{eq:virial-series-pressure} can be thought of as the deviation from ideal gas behaviour, because $\beta p = \rho$ is the ideal gas law.}
\begin{subequations}
  ...
  \end{equation}
\end{subequations}
where $B_k$ are the virial coefficients
\begin{subequations*}
  \begin{equation}
    B_k =
    -\frac{k-1}{k!}
    \sum \textrm{all }\textit{stars}\textrm{ with 1 root and } k-1 \textrm{ field points}.
  \end{equation}
\end{subequations*}
...
The excess free energy per particle is determined from the equation of state by
\begin{subequations}
  \begin{equation}
    \frac{\beta F^{ex}}{N} =
    \int_0^\eta \left( \frac{\beta p}{\rho} - 1 \right) \, \frac{d\eta'}{\eta'} =
    \sum_{k=2}^\infty
    \frac{1}{k-1} B_k \, \rho^{k-1},
  \end{equation}
\end{subequations}
or equivalently expressed per volume
\begin{subequations*}
  \begin{equation}
    \frac{\beta F^{ex}}{V} =
    \rho \frac{\beta F^{ex}}{N} =
    \sum_{k=2}^\infty
    \frac{1}{k-1} B_k \, \rho^k,
  \end{equation}
\end{subequations*}
giving the a few equivalent expressions of the excess chemical potential
\begin{equation}
  \begin{split}
    \beta \mu^{ex} &=
    ...
    &=
    \sum_{k=2}^\infty
    \frac{k}{k-1} B_k \, \rho^{k-1}
    =
    \sum_{k=1}^\infty
    \frac{k+1}{k} B_{k+1} \, \rho^{k}.
  \end{split}
\end{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
with the first few composition independent coefficients
\begin{subequations}
  \begin{align}
    B_{1,1} &=
    -\frac{1}{2!}
  ...
  \end{align}
\end{subequations}
Giving the excess cost of insertion as
\begin{equation}
  \begin{split}
    \beta \Omega^{ex} &= \beta \mu^{ex}_2 - \beta p \Delta V \\
    &=
    \sum_{k=1}^\infty
    \left(
    \frac{k+1}{k}
    ...
    &=
    \sum_{k=1}^\infty
    \beta\Omega^{ex}_k \rho^k.
  \end{split}
\end{equation}
where the first few terms in the density expansion are
\begin{subequations}
  \begin{align}
    \beta\Omega^{ex}_1 &=
    - \mayerdiagram[|][t.]{2}
  ...
  \end{align}
\end{subequations}
We argue that $\beta\Omega^{ex}_1 = 0$ below, and for the second argument using the Ree-Hoover replacement rules we have
\begin{equation}
  \begin{split}
    \beta\Omega^{ex}_2
    &=
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Note to get the coefficients in front of the brackets we used
\begin{equation*}
  \frac{k + 1}{k} \frac{k}{(k+1)!} = \frac{1}{k!}.
\end{equation*}

The starting point for geometrical approaches is a recognition that for two hard bodies $A, B \in \mathcal{K}^d$ the Mayer-$f$ function reduces to
\begin{equation}
  f(A, B) =
  \begin{cases}
    0 & \qquad A \cap B = \emptyset\\
    -1 & \qquad A \cap B \ne \emptyset
  \end{cases}
\end{equation}
for convex bodies this is simply
\begin{equation}
  f(A, B) = - \mu_0(A \cap B)
\end{equation}
\todo{Comment on how these approaches break down for highly elongated shapes, \cite{Wertheim1994}.}
For hard spheres this is particularly simple reducing to
\begin{equation}
  f(A, B) =
  - \Theta \left(
  \frac{\sigma_A + \sigma_B}{2} - |\vec{r}|
  \right)
\end{equation}
where $\vec{r}$ is the separation between the centers of $A$ and $B$ and $\sigma_{\{A,B\}}$ their diameters.
