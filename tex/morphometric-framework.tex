...
Note that in general $y^{(n)}(\vec{r}^n) > 0$, so as long as we just need to find some minimal set of configurations where $V(\mathbb{V}_R')$ is nonzero and scales as $\mathcal{O}(R^{nd})$ in order to complete the lower bound.
...
\end{equation}
which reduces to an accessible volume in the limit that $\phi$ describes a hard interaction.\todo{Can logarithmic surface tension corrections (capillary waves) destroy the formal result?}
