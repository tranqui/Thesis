...
Ordinarily the end result of these approaches is an equation of state for the pressure $p = p(\rho)$, which gives the free energy implicitly through the thermodynamic relation
...
\subsection{Mixtures: truncatability of the free energy}

\todo{Port in stuff from results chapters and discuss.}

\subsection{Virial series}
\label{sec:virial-series}

The virial series is the only properly rigorous approach to evaluating the partition function, and thus the free energy, from first principles.
...
Alternatively, the virial series can be expressed for the (excess) free energy density as
\begin{equation}\label{eq:virial-series-excess-free-energy}
  ...
  \rho^n
\end{equation}
which is more useful for connecting with density functional theory (section \ref{sec:dft}).
\todo{Specify which thermodynamic relation.}
...
In the above expression we used the $\{\cdot\}$ notation which we introduced to indicate different permutations in the various standard correlation functions e.g.\ \eqref{eq:cluster-correlation-functions}; this does not come as a surprise because the integrand above is really another correlation function.
\todo{Fix clumsy wording.}
...
First step is to reduce the number of diagrams.
We use the same trick as before: we treat the grand canonical partition function as a moment generating function, with fugacity as the control parameter, and consider its the dual cumulant generating function $\ln{\Xi} = -\beta\Omega$.
It follows that
\begin{equation*}
...
\marginfootnote{These are also sometimes called \emph{Ursell functions}.}
$W^{(n)}$.
...
\end{equation*}
The volume factor is pulled in front of the definition of $b_n$ because one obtains a volume factor from translation invariance.

\todo{Check density expression is consistent with DFT section.}
\begin{tcolorbox}[title=The virial coefficients]
  Lastly, we transform to an expansion in density.
  We then convert to pressure to get the irreducible cluster integrals (sometimes called stars, not to be confused with star diagrams of graph theory, 2-connected).
  ...
  \end{equation*}
  This expression, connecting density and fugacity, can be used to transform the expansion in fugacity to one in density i.e.\ \eqref{eq:virial-series-pressure}.
  This gives the first few virial coefficients
  \begin{subequations}
    \begin{align*}
      B_2 &= -b_2
      \\
    ...
    \end{align*}
  \end{subequations}
  Further cancellations occur after inserting the explicit expressions for the cluster integrals \eqref{eq:cluster-integral} leaving only the \emph{stars}%
