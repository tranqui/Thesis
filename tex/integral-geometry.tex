...
Here we introduce the \emph{kinematic formulas}, which calculate the probability that randomly distributed objects hit one another.
...
The intersection of convex objects is also convex, so
\begin{equation*}
  \chi[K_1 \cap K_2]
  =
  \begin{cases}
  ...
  \end{cases}
\end{equation*}
and the probability that the two objects collide is then
\begin{equation*}
  \mathrm{Prob}[\textrm{collision}]
  =
  \frac{1}{V} \int_V \chi[K_1 \cap K_2(\vec{r})] \, d\vec{r}
\end{equation*}
where $K_2$ is uniformly distributed and $K_1$ acts as a fixed target.
In this integral $K_2$ is translated over the space, however in general these objects will be non-spherical so it is more natural to consider also their relative orientation.
Integral geometry more naturally deals with integrations over relative positions and orientations, at the small cost of additional notation.
...
\begin{equation*}
  g K := \{\mathcal{R}(\vec{\theta}) \vec{k} + \vec{r} \, | \, \vec{k} \in K\},
  %(\mathcal{T} \circ \mathcal{R})(\vec{r}, \vec{\theta}),
\end{equation*}
a member of the rigid motion group $g \in G_d := \mathbb{R}^d \times \mathrm{SO}(d)$, and where $\mathcal{R}(\vec{\theta}) \in \mathrm{SO}(d)$%
...
\end{equation*}
if they now occupy all of Euclidean space.
...
\begin{tcolorbox}[title=Principal kinematic formula]
  ...
  \end{equation}
  We see the flag coefficients \eqref{eq:flag-coefficients} play an analogous role in conjugating the intrinsic volumes here as binomial coefficients do in algebraic expansions%
