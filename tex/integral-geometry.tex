...
The usefulness of this particular focus is directly connected to the concept of \emph{extensivity} in statistical mechanics, which we will use to guide the following discussion.
...
%% \end{equation}
%Then,
We will call these quantities \emph{volume-extensive}.
As an example of a volume-extensive quantity, the \emph{entropy} can be written
\begin{equation*}
  S = s V
\end{equation*}
where $s$ would be an entropy density which is intensive, i.e.\ it does not change with system volume.
...
As an example of curvature in statistical mechanics, an early example is the Young-Laplace equation for the pressure change between two fluids
\begin{equation}
  \Delta p = \gamma \Tr{\kappa}
\end{equation}
where $\kappa$ is the curvature tensor describing the shape of the phase boundary.
