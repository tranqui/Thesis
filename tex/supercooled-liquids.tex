...
\marginfootnote{In Maxwell's viscoelasticity theory, these are linearly related with constant of proportionality $G_\infty$, the high frequency shear modulus.
As such, these can be thought of as equivalent quantities.}.
...
\begin{SCfigure}
  ...
  \caption[Intermediate scattering function in a supercooled liquid]{
    Typical intermediate scattering functions in a supercooled liquid, for values of $k \sim 2\pi / \sigma$ where $\sigma$ is the interaction lengthscale.
...
\marginfootnote{This process is imperfect, as even highly polydisperse systems have been found to crystallise without careful fine tuning of the size distribution \cite{BommineniPRL2019,BerthierPRL2016,SanzPRL2011}.
  However, crystallisation of polydisperse systems occurs through \emph{fractionation} rather than nucleation.}
...
\marginfootnote{That is, the material which the average person would mean when they say ``glass''.}[-0.5cm],
...
\marginfootnote{This baffling naming convention has nothing to do with the mechanical properties.}[-0.5cm]
glassformers.

\begin{SCfigure}
  \includegraphics[width=0.9\linewidth,outer]{angell}
  \caption[Angell plot for model systems undergoing dynamical arrest]{
    \emph{Angell} plot \cite{AngellJNS1988} for molecular and model glassformers showing the temperature/pressure dependence of viscosity (labelled here as $\eta$) or equivalently relaxation time $\tau_\alpha$.
    ...
    The compressibility factor $Z = \beta p / \rho$ is argued to be equivalent to inverse temperature for hard spheres (HS) \cite{BerthierPRE2009}, with data taken from Ref.\ \cite{RoyallJSM2017}.
...
As we argued in the introduction, pressure%
\marginfootnote{Equivalent to density, with the connection made through the equation of state $p = p(\rho)$.}[-0.5cm]
is the only meaningful state variable.
...
\marginfootnote{The leading order behaviour can be given more justification by invoking the morphometric approach \eqref{eq:fmt-morphometric-2}, or from more general arguments which we will introduce in section \ref{sec:insertion-as-solvation}.}[-1cm]
we then expect
\begin{equation*}
...
\begin{SCfigure}
  ...
  \caption[Dynamical heterogeneities in binary hard discs]{
    Dynamical heterogeneities in a binary hard disc system with a size ratio of $1:1.4$.
...
Dynamical heterogeneities are now seen as quintessential features of the glass transition \cite{BerthierRMP2011,BiroliJCP2013}, so the origin of the growing dynamical length would be a necessary component for any theory to be accepted.
%The size of these hetereogeneities is captured by a dynamical lengthscale $\xi_\mathrm{dyn}$, whereas thermodynamic theories typically concern static lengthscales.
...
The former viewpoint includes the related \emph{mode-coupling} and \emph{random first-order transition} theories, which are connected in the mean-field picture of glasses.
...
\end{equation}
with%
\marginfootnote{As a reminder to the reader, the $c^{(2)}$ appearing here is the two-body direct correlation function defined through \eqref{eq:direct-correlations}.}
\begin{subequations}\label{eq:full-mct-coupled}
  \begin{align}
    ...
    &=
    \frac{
      \vec{k} \cdot \vec{q} \, c^{(2)}(q)
      + \vec{k} \cdot (\vec{k} - \vec{q}) \, c^{(2)}(|\vec{k} - \vec{q}|)
    }{
      k
    }.
  \end{align}
...
This method requires knowledge of the structure factor, which implicitly depends the state point.
Together \eqref{eq:full-mct} and \eqref{eq:full-mct-coupled} form a non-linear coupled set of integro-differential equations that can be numerically solved, providing a reasonable description of the first few decades of dynamical slowdown, with deviations only becoming noticeable at deep supercooling \cite{FlennerPRE2011,BrambillaPRL2009}.
...
However, this predicted transition has long been known to be spurious as the theory does not allow for activated dynamical events.
Moreover, recent experiments have managed to equilibrate colloidal hard sphere liquids up to $\eta \lesssim 0.60$ \cite{BrambillaPRL2009,HallettNC2018} and to even higher densities in simulations \cite{BerthierPRL2016}.
Despite this failing of MCT, it remains the only properly first-principles theory for treating liquid dynamics in physical dimensions%
\marginfootnote{In higher dimensions, very similar theories emerge from mean-field theory \cite{MaimbourgPRL2016,SzamelPTEP2013}.}.
...
\end{equation}
with $S_\mathrm{vib}$ as the \emph{vibrational entropy}, though some refinement is required for polydisperse systems \cite{OzawaJCP2017,OzawaJCP2018} and where additional non-vibrational processes exist that do not relax the density profile \cite{OzawaPRL2018}.
The exact definition of $S_\mathrm{conf}$ thus depends on how one defines $S_\mathrm{vib}$, which can be defined in multiple distinct, though broadly equivalent, ways.
For example, a common convention is to define the vibrations as all those excitations around an energy minimum \cite{Wales2004}; we will use this convention in chapter \ref{chapter:morphometric-applications}.
...
\begin{equation}\label{eq:rfot-barrier}
  \Phi(\xi) \sim \gamma_\mathrm{eff} \xi^\theta - T s_\mathrm{conf} \, \xi^d.
\end{equation}
The maximum of $\Phi$ occurs at the \emph{point-to-set} length
...
Static lengthscales%
\marginfootnote{There are multiple ways of extracting lengthscales from the LFS, see e.g.\ Refs.\ \cite{CharbonneauPRL2012,CharbonneauPRE2013,HallettNC2018}.}
...
They found that the collective dynamical behaviour was dominated by the LFS for small frustration, as expected, but that increasingly the static and dynamical lengthscales decouple with strong frustration \cite{TurciPRL2017}.
...
In this picture repeating patches in the liquid would be interpreted as indicative of a growing amorphous order \cite{KurchanJPMT2011}.
...
%Charbonneau: growth of length scale associated with LFS does not correlate with dynamical length scale \cite{CharbonneauPRL2012,CharbonneauPRE2013}, possibly multiple relaxation mechanisms or a non-static length scale.
...
%% It is possible that low-dimensional fluctuations prevent any thermodynamic transition, and it has been argued that even if there is a thermodynamic transition at $\eta_K$ this may not be responsible for the dynamical slowdown seen around the operational glass transition $\eta_g \ll \eta_K$ \cite{WyartPRL2017}.
%% As such, there is room for alternative pictures to the mean-field/RFOT scenario.

The main opposition theory to the thermodynamic scenarios outlined above is \emph{dynamical facilitation}, which focuses on the role of kinetic constraints.
...
Inspired by this, dynamical facilitation posits that relaxation in supercooled liquids occurs through a series of connected elementary excitations \cite{ChandlerARPC2010,SpeckJSM2019}.
Dynamical heterogeneity and its associated lengthscale are the fundamental features of glass in this picture, and no thermodynamic phase transition occurs so the relaxation time remains finite until $T = \SI{0}{\kelvin}$.
The dynamical lengthscale corresponds to the distance between dynamical defects in this theory, which could have a structural interpretation although the nature of the defects are usually considered incidental to the theory.
Notably, the agreement of dynamical facilitation with available structural relaxation time data is comparable with the mean-field/RFOT scenario up to the experimental glass transition \cite{ChandlerARPC2010}.
However, dynamical facilitation fails to explain other observables such as the reduction in $S_\mathrm{conf}$ seen in Fig.\ \ref{fig:swap-sconf}.
...
In subsequent chapters we will heavily focus on the treatment of static many-body correlation functions in the bulk liquid.
...
Even supposing the existence of a transition, the precision required to detect such a signal in $g^{(2)}(r)$ may be arbitrarily subtle.
...
\begin{subequations}
  \begin{align}
    ...
    &=
    k_B \, \rho \left( 1 + \frac{d}{2} - \ln{\rho \Lambda^d} \right),
    \\
...
By setting the number of particles in the correlation function we effectively fix the liquid lengthscale.%, so we can hopefully start to address some of the questions outlined here.
