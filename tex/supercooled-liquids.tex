...
\subsection{Mean-field theories}
...
The existence of the Gardner phase provides context for the importance of mean-field theory, but it cannot be directly connected to our approach in subsequent chapters so we will leave it here.
...
The relevance of mean-field descriptions in physical dimensions requires a finite-dimensional description of the supercooled liquid.
...
\subsection{Finite-dimensional theories}
...
The complexity is thus not well-defined, as there will not be strict separations of metastable states, though they may be long-lived; the dynamical transition of mean-field thus becomes a cross-over.
In finite dimensions the \emph{configurational entropy} $S_\mathrm{conf}$ is introduced as the closest equivalent to the complexity; this is not uniquely defined, but a common definition invokes the timescale separation being vibrational ($\beta$--) and liquid-like ($\alpha$--) dynamics.
The configurational entropy then describes the entropy of the latter processes, obtained as the residual entropy after removing vibrations
...
The RFOT scenario considers the effect that a depleting $S_\mathrm{conf}$ would have on the dynamics.
\hl{(argument: RFOT)}
\hl{Papers on finite dimensions:} \cite{BouchaudJCP2004,DzeroPRB2005,FranzJSM2005,AngeliniJSP2017,RulquinJSM2016,BiroliMeanPRB2018,BiroliFinitePRB2018}
This argument has been rigorously proven in Ref.\ \cite{MontanariJSP2006}.
Moreover, they prove that any diverging relaxation time \emph{must} coincide with an entropy crisis $S_\mathrm{conf} \to 0$.
Measurements of the configurational entropy in deeply supercooled hard spheres suggest a vanishing $S_\mathrm{conf}$ at finite $\eta$ in $d=3$ (Fig.\ \ref{fig:swap-sconf}), pointing to an RFOT scenario.
...
This picture starts from the observation that crystal structures (e.g.\ face-centred cubic) are generally not the optimal energetic arrangement for small clusters of particles, even though they must become so in bulk systems.
For example, isolated Lennard-Jones atoms will preferentially arrange as vertices of a five-fold symmetric \emph{icosahedron} \cite{FrankPRS1952} (Fig.\ \ref{fig:frustration}b).
As five-fold symmetric structures cannot tesselate Euclidean space, Sir Charles Frank conjectured that glassformers suppress crystallisation by having a propensity to form these \cite{FrankPRS1952}.
\hl{Notably, in hard spheres tetrahedral structures are typically the densest way to pack for small clusters.
They pay a cost as the clusters enlarge, as they cannot tesselate space, so they are said to be} \emph{frustrated}.
This viewpoint was developed into the more complete \emph{frustration-limited domain theory} \cite{TarjusJPCM2005}, \hl{the glass is seen as .}
This picture is broadly compatible with the mean-field viewpoint: the propensity for a system to form certain structures can be interpretted as a manifestation (or a driver) of a reduced $S_\mathrm{conf}$.
...
Recently, it has been shown in numerical simulations that \emph{competition} between different favoured structures enhances the propensity for glassformation \cite{TeichNC2019}.
...
Recent numerical evidence suggests that a model glassformer in $d=2$ does not show a transition, also vanishing at $T=0\si{K}$ \cite{BerthierNC2019}, pointing to a lower critical dimension of at least $d_L \ge 2$.
...
\subsection{Perspective: usefulness of many-body correlations}
...
And even supposing the existence of a transition, the precision required to detect such a signal $g(r)$ may be arbitrarily subtle; however, any changes approaching a transition \emph{must} be magnified in the higher order correlation functions because of \eqref{eq:correlation-derivatives}.
...
In principle, our real space correlation functions can be Fourier transformed to obtain these, but in practice this is prohibitively expensive because of the high dimensionality of the arguments for even modest $n$.
