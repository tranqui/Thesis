...
\hl{Having concluded the phenomenology of supercooled liquids we can now proceed to specific theories of the glass transition.}
...
\begin{equation}\label{eq:general-langevin}
  \frac{d \vec{\tilde{X}}(\vec{k}, t)}{dt}
  =
  i \vec{\Omega} \cdot \vec{\tilde{X}}(\vec{k}, t)
  - \int_0^t \vec{K}(t') \cdot \vec{\tilde{X}}(\vec{k}, t - t') \, dt'
  + \vec{f}(t)
\end{equation}
...
This requires knowledge of the memory function $\vec{K}$, which must be approximated in general.
%% \begin{equation*}
%%   \vec{K}(t') = \vec{f}(0)^T \vec{f}(t') \cdot (\vec{\tilde{X}}^T \vec{\tilde{X}})^{-1}.
%% \end{equation*}

\hl{Through a series of algebraic steps, (NOPE)} it is possible to construct an evolution equation for the intermediate scattering function \eqref{eq:isf} from the generalised Langevin equation \eqref{eq:general-langevin}.
The full, as \cite{ReichmanJSM2005}
\begin{equation}
...
Crucially, MCT predicts a diverging timescale at $\eta \sim 0.52$ in hard spheres, where the dynamics would become completely arrested; corresponding numerical fits with the same power law to colloidal experimental data move this point to $\eta \sim 0.58$ \cite{VanMegenPRE1994}.
However, these predicted transitions are spurious as recent experiments have managed to equilibrate colloidal hard sphere liquids up to $\eta \lesssim 0.60$ \cite{BrambillaPRL2009,HallettNC2018} and in simulations to even higher densities \cite{BerthierPRL2016}.
Despite this failing of MCT, it remains the only properly first-principles theory for the treating liquid dynamics.
Moreover, MCT provides a reasonable description of the \emph{onset} of dynamical arrest, with deviations only becoming noticeable at deep supercooling \cite{FlennerPRE2011,BrambillaPRL2009}.

Extensions incorporating fluctuations can be found in Refs.\ \cite{BiroliPRL2006,SzamelPTEP2013}, and more recently a generalised MCT has been developed which avoids the uncontrolled factorisation of the pair densities by incorporating in closures of the many-body correlation functions \cite{JanssenPRL2015,JanssenFP2018}.
This latter approach was able to move the diverging timescale to higher densities, bringing the theory in good agreement with available data from simulations and experiments.
...
In principle, our real space correlation functions can be Fourier transformed to obtain these functions, but in practice this is prohibitively expensive because of the high dimensionality of the arguments for even modest $n$.
