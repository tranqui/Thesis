...
My main contribution was in theory and simulations to better understand the experimental data.
Consequently, while I will describe the experiments the focus of the chapter will be on the theory and numerics.
...
In addition, atmospheric aerosols have potential application in geoengineering \cite{PringleACP2012}.
...
Understanding the droplet drying and crystallisation is also important for industrial applications, most notably in spray-drying.
The goal in these applications is to control the distribution of sizes, morphology and phase of the final droplets, which are very sensitive to processing conditions such as solvent \cite{CarverIECR2012,LintingreSM2016}, temperature \cite{IveyAST2018,YouDT2014,LinPT2015}, pH \cite{YuJPS2002,DubbiniIJP2014} and co-spray dried components \cite{ZhongAP2018,NandiyantoAPT2011,LyuJCG2017}.
Tailoring crystallisation is particularly important because of crystal and amorphous states have fundamentally different properties: crystalline droplets are typically more stable, suitable for e.g.\ long-term storage \cite{VehringJAS2007,CostantinoJPS1998}, whereas amorphous droplets are more soluble which is desirable for e.g.\ drug delivery \cite{AmstadJPCB2016,BroughIJP2013}.
...
\todo{I copied this section pretty closely from Flo, so probably needs a lot of rewording}

In this work we will investigate drying and crystal nucleation of free aerosol droplets, by combining experiments and numerical simulations of free aerosol droplets.
...
The kinetics of drying \ce{NaCl} and \ce{NaNO3} droplets was measured using Comparative-Kinetics Electrodynamic Balance (CK-EDB).
In all experiments HPLC-grade water, BioXtra $\ge 99.5\%$ \ce{NaCl} (Sigma-Aldrich) and $X \%$ \ce{NaNO3} \hl{(brand)} were used.
The CK-EDB instrument has been detailed in previous work\cite{DaviesAST2012,DaviesAC2013} \hl{(which is correct reference?)} so we will only describe it briefly here.
...
Upon generation the droplets are charged ($<\SI{10}{\femto\coulomb}$ through e.g.\ an ionic imbalance) with an induction electrode so they can be levitated by application of a DC field to counteract gravity.
An AC field is applied in the perpendicular direction to trap the droplet.
...
For isotropic droplets in an amorphous state the droplet radius $R$ determines the angular separation between the fringes in the pattern $\Delta \theta$.
...
Nucleation and growth occur on such a short time scale that it is not possible to obtain information from the experiments on where inside the droplet this occurs or how many initial nucleation sites there were; we can only determine that the droplet has crystallised.
...
Liquid \ce{NaCl}--\ce{H2O} and \ce{NaNO3}--\ce{H2O} droplets were placed in the instrument at constant RH for a short time to ensure well-mixed initial droplets, after which the RH was dropped to initiate drying.
The switches in RH occur rapidly over $\SI{\sim0.1}{\second}$.
Crystallisation of \ce{NaCl} droplets occurred within $\SI{\sim1.7}{\second}$, whereas \ce{NaNO3} droplets showed stochastic behaviour with a fraction of droplets not crystallisation over the timescale of the experiment.
The stochastic behaviour persists when the experiment was repeated for the \emph{same} droplet over a cycle of lowering and raising the RH, ruling out impurity-driven heterogeneous nucleation.
\todo{Should probably cite previous paper somewhere?}

\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.75\linewidth]{aerosol-droplet}
  \end{minipage}
  \caption{A drying droplet solution of radius $r=R(t)$ surrounded by a gas of temperature $T_\infty$ and relative humidity RH.
...
\begin{equation}\label{eq:species-continuity}
  ...
  \qquad i \in \{f,s\}
\end{equation}
where $\vec{v}^{(i)}$ is the velocity of species $i$.
...
This simplifies the advective term in \eqref{eq:species-continuity-relative} leading to
...
\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{aerosol-diffusion-fit}
  \end{minipage}
  \caption{Numerical fits of binary diffusion coefficients for aqueous ionic solutions.
...
We employ two simplifying in our calculations concerning the effects of droplet temperature.
...
Initially the droplets are prepared in equilibrium, so they are well-mixed solutions and we can assume an initial uniform concentration profile.
At $t=0$ the humidity is dropped leading to evaporation of the solvent.
The evaporation rate determines the boundary conditions for \eqref{eq:final-diffusion}.

Integrating \eqref{eq:species-continuity} gives the total mass flow into the droplet (of each species) as
...
\begin{equation}\label{eq:radial-evolution}
  \frac{dR}{dt} =
  \frac{1}{4\pi R^2 \rho^{(f)}_0} \frac{dm^{(f)}}{dt}
\end{equation}
so we need a model of the evaporation rate $\tfrac{dm^{(f)}}{dt}$ to close this system of equations.
...
%Gas phase assumptions: spherical symmetry, ideal gas, quasi-steady evaporation.
We assume the classic result for quasistatic vaporisation\cite{Slattery1999,Sirignano2010}
\begin{equation}\label{eq:vaporisation}
  \frac{dm^{(f)}}{dt}
  =
  4\pi \rho_v D_v R \ln{(1 + B)}.
\end{equation}
where $\rho_v$ and $D_v$ are the density and diffusion constant in the vapour phase, and the \emph{Spalding number} is defined as
\begin{equation}\label{eq:spalding}
  B =
  \frac{\displaystyle{\lim_{r \to \infty}} Y^{(f)}(r) - Y^{(f)}(R^+)}
       {1 - \displaystyle{\lim_{r \to \infty}} Y^{(f)}(r)}.
\end{equation}
with $Y^{(f)}(R^+)$ indicating that it is the mass fraction of the solvent component on the vapour side of the boundary.
...
\end{equation}
where $p_f(R^\pm)$ are the partial pressures on either side of the surface which we relate to the mass fractions using Raoult's law in the gas phase and experimental fits of the solvent activity \hl{(do we need to say where from?)}.
...
Together, Eqs.\ \eqref{eq:vaporisation}, \eqref{eq:spalding}, \eqref{eq:molar-fraction} and \eqref{eq:temperature-difference} form a complete set of equations that can be solved (numerically) to obtain the evaporation rate.

Typically the classic vaporisation rate equation \eqref{eq:vaporisation} requires semi-empirical corrections to treat more complex mass and heat transport phenomena at the boundary.
...
\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{nano3-trajectory}
  \end{minipage}
  \caption{Evolution of \ce{NaNO3} aerosol droplet radii from simulations (dashed lines) and experiments shown by points with 1\% transparency, showing reasonable agreement at short times until longer times when the evaporation rate is underestimated.
...
\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{aerosol-cnt}
  \end{minipage}
  \caption{Shell nucleation rate $J\xi$ (\si{\per\micro\metre\squared\per\second}) predicted by classical nucleation theory for aqueous \ce{NaNO3} and \ce{NaCl} solutions at different state points.
...
We define the surfival probability as
\begin{equation*}
  p_\mathrm{liq}(t)
  :=
...
\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{nacl-trajectory}
  \end{minipage}
  \caption{Evolution of \ce{NaCl} droplets in dry air $\mathrm{RH}=0\%$ from experiments (points) and simulations (lines) for different ambient temperatures and initial solute mass fractions.
...
\begin{SCfigure}
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{nano3-nucleation-rates}
  \end{minipage}
  \caption{State points explored by experiments with drying \ce{NaNO3}--\ce{H2O} aerosol droplets as determined from simulation for 9 datasets for different initial conditions.
    ...
    Bottom panel: corresponding nucleation rates assuming boundary-dominated nucleation \eqref{eq:droplet-nucleation-rate} showing non-monotonic behaviour in increased concentration and temperature in contrast with the predictions of classical nucleation theory in Fig.\ \ref{fig:cnt}.}
...
Combined with the numerical model, which gives us the precise state of the droplet, we can infer $J\xi$ from inversion of the rate formula \eqref{eq:droplet-nucleation-rate} under the assumption that nucleation is boundary-dominated.
...
\begin{equation}
  ...
  - W p_\mathrm{liq},
\end{equation}
upon combining this with our assumption that nucleation occurs near the boundary \eqref{eq:droplet-nucleation-rate} allows to write the nucleation rate as
\begin{equation}
  J\xi
  =
...
The diffusion limit would persist even in more sophisticated transport phenomena were modelled in the evaporation model.
Surface enrichment is overestimated because we have neglected the effect of temperature gradients inside the droplet, and because we have used an extrapolation of low concentration diffusion data which is likely underestimates diffusion at high concentrations.
...
However, this can only partially explain the observed behaviour because of the extreme sensitivity of CNT to concentration.
