...
\todo{Should expand this a little. May not write anything new.}
...
\begin{equation*}
  \mathcal{N} =
  \int_{\mathcal{Q}} \rho^n
  \exp{\left(-\beta\phi^{(n)}(\vec{r}^n)\right)}
...
\begin{equation}
  \mathcal{N} =
  \rho^n V \int_\mathcal{D} g^{(n)}(\vec{r}^{n-1}) \, d\vec{r}^{n-1},
\end{equation}
...
We will begin by proposing a definition for the structures so that we have a domain of integration.
\todo{Reference specific sections}
...
A structure is rigid if the only degrees of freedom are rigid-body motions i.e.\ translations and rotations.
...
These ideas are based on linear algebra where each particle coordinate is a degree of freedom and each contact is a constraint.
...
Curiously, the overwhelming majority of structures appear to satisfy the Maxwell criterion \eqref{eq:maxwell-criterion} \cite{Holmes-CerfonSR2016,Holmes-CerfonARCMP2017} despite the linear theories being inexact.
...
In the fully non-linear theory, rigidity is classified into three categories: \cite{ConnellySJDM1996,Holmes-CerfonARCMP2017}
\begin{enumerate}
...
It would be interesting to extend the current work to these other structures to explore the full library of local structures.
...
\marginfootnote{There is no elegant step omitted here to obtain the form of $\vec{U}$ which gives block diagonal form; rather, I spent an hour guessing its form inside Mathematica until I hit upon this result.
...
\marginfootnote{We are skipping an important step here, because it requires formalism to be introduced in section \ref{sec:bond-distance}.
  Specifically, this integral must be done in \emph{bond-distance space}, i.e.\ we perform the multivariate Gaussian integral in the space of $\{y_1, \cdots, y_m\}$.
  Evaluating this Gaussian over all of space i.e.\ $y_k \in [-\infty, \infty]$ introduces a small approximation as we have the strict lower bound $y_k > -\sigma$ from \eqref{eq:bond-distance}.}.
...
\todo{Explain what is meant by first-order and nonsingular here.}
...
\todo{Connect with five-fold symmetry calculation later.}
